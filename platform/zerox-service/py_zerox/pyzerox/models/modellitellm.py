...
from ..core.langfuse import langfuse
...
class litellmmodel(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :param schema: JSON schema to enforce structured output format
        :type schema: Dict[str, Any], optional
    ...
    async def completion(
        ...
        if self.schema:
            if "gemini" in self.model.lower():
                response_format = {"type": "json_object", "response_schema": self.schema.get("json_schema").get("schema"), "strict": True}
            else:
                response_format = {"type": "json_schema", "json_schema": self.schema.get("json_schema")}

        try:
            response = await litellm.acompletion(
                model=self.model, 
                messages=messages,
                **({"metadata": {"trace_id": generation_id}} if generation_id else {}),
                **({"response_format": response_format} if self.schema else {}),
                **self.kwargs
            )
