...
def extract_bookmarks(pdf_path: str) -> List[Dict[str, Any]]:
    """
    Extracts bookmarks (outlines) from a PDF file if they exist.
    
    :param pdf_path: Path to the PDF file.
    :type pdf_path: str
    :return: A list of dictionaries containing bookmark information.
             Each dictionary has keys: 'title', 'page_number', 'level', 'parent_id'.
             Returns an empty list if no bookmarks exist.
    ...
    with open(pdf_path, "rb") as pdf_file:
        ...
        # Check if the PDF has outlines/bookmarks
        if not reader.outline:
            return result
        
        # Process bookmarks recursively
        def process_bookmarks(bookmarks, level=0, parent_id=None):
            bookmark_list = []
            
            for i, item in enumerate(bookmarks):
                if isinstance(item, list):
                    # This is a list of child bookmarks
                    # The parent is the previous item
                    if i > 0 and not isinstance(bookmarks[i-1], list):
                        parent_bookmark_id = len(result) - 1
                        child_bookmarks = process_bookmarks(item, level + 1, parent_bookmark_id)
                        bookmark_list.extend(child_bookmarks)
                else:
                    # This is a bookmark item
                    try:
                        page_number = reader.get_destination_page_number(item)
                        # Convert to 1-indexed page number to match the rest of the codebase
                        page_number += 1
